...
export default function IdentityForm() {
  ...
  const [duplicateWarning, setDuplicateWarning] = useState<{
    ...
    reasons: string[];
  }>({ visible: false, existing: null, newData: null, reasons: [] });
  ...
  useEffect(() => {
    store
      .getSettings()
      .then((s) => setIsRegistrationOpen(s?.isRegistrationOpen !== false));
  }, []);
    ...
    return (
      <div className="p-12 flex flex-col items-center justify-center bg-white rounded-2xl min-h-[400px]">
        ...
        <p className="text-slate-500 text-center mb-6">
          Mohon maaf, sesi pendaftaran telah berakhir atau ditutup oleh Admin.
        </p>
        <button
          onClick={() => navigate("/")}
  ...
  if (!isLoaded || !attendee || isRegistrationOpen === null) {
    return (
      <div className="p-12 flex flex-col items-center justify-center bg-white rounded-2xl min-h-[400px]">
        ...
        <h2 className="text-base text-slate-500 font-medium tracking-tight">
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setIsChecking(true);
    const existingNpk = await store.getAttendeeByNpk(data.npk);

    let dupRecord = existingNpk;
    let dupReasons: string[] = [];

    if (existingNpk && existingNpk.id !== attendee.id) {
      dupReasons.push("NPK / Akun Siaga");
    } else {
      const allDict = await store.getAllAttendees();
      const otherAttendees = Object.values(allDict).filter(
    ...
    if (dupRecord && dupRecord.id !== attendee.id) {
  ...
  return (
    <div className="p-6 md:p-8 animate-in fade-in duration-300">
      <div className="flex items-center gap-3 border-b border-slate-100 pb-5 mb-6">
        <span className="text-teal-600 font-mono text-sm border border-teal-200 bg-teal-50 px-2 py-0.5 rounded-md">
          STEP_01
        </span>
        <div>
          ...
          <p className="text-slate-400 text-[13px] leading-snug">
            Mohon lengkapi profil Anda dengan data yang valid.
          </p>
        </div>
      </div>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6 flex gap-3 text-slate-800 animate-in slide-in-from-top-4 duration-300">
        ...
        <div>
          <h4 className="font-bold text-amber-900 text-xs sm:text-sm uppercase tracking-wider mb-1">
            Peringatan Penting: Hindari Mengisi Double
          </h4>
          <p className="text-xs sm:text-[13px] leading-relaxed text-amber-800">
            Setiap peserta hanya diperkenankan memiliki{" "}
            <strong>satu data registrasi</strong>. Jika Anda sudah pernah
            mendaftar dan ingin melakukan penyesuaian jawaban atau mengunggah
            bukti baru, Anda cukup mengisi NPK Anda. Sistem akan otomatis
            mendeteksi dan menawarkan opsi <strong>"Ubah/Edit Jawaban"</strong>{" "}
            untuk melanjutkan data lama Anda agar tidak terjadi penumpukan data
            ganda.
          </p>
        </div>
      </div>
          ...
          <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl border border-slate-100 overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 text-center">
              ...
              <p className="text-slate-500 text-xs sm:text-sm leading-relaxed mb-4">
                Sistem mendeteksi bahwa data Anda atas nama{" "}
                <strong className="text-teal-700 capitalize">
                  {(duplicateWarning.existing?.fullName || "").toLowerCase()}
                </strong>{" "}
                kemungkinan besar sudah terdaftar sebelumnya.
              </p>

              <div className="bg-amber-50/50 rounded-xl p-3.5 mb-6 text-left border border-amber-100/40 space-y-2">
                <p className="text-[11px] sm:text-xs text-amber-900 leading-normal font-bold border-b border-amber-200/50 pb-2">
                  Kesamaan Data Ditemukan Pada:
                </p>
                <div className="flex flex-wrap gap-1.5">
                  {(duplicateWarning.reasons || ["NPK / Akun Siaga"]).map(
                    (r) => (
                      <span
                        key={r}
                        className="text-[9px] font-bold uppercase tracking-wider bg-white border border-amber-200 text-amber-700 px-2 py-0.5 rounded-full inline-block"
                      >
                        {r}
                      </span>
                    ),
                ...
                <p className="text-[11px] sm:text-[11px] text-amber-800 leading-normal font-medium mt-2 pt-2 border-t border-amber-200/50">
                  ⚠️ <strong>Pilihan Aman (Agar Tidak Double Data):</strong>{" "}
                  Anda disarankan memilih tombol edit jawaban di bawah untuk
                  memutakhirkan data pendaftaran Anda yang lama (
                  {duplicateWarning.existing?.npk}), menjaga tertib
                  administratif panitia.
                </p>
              </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label className="flex items-center gap-2 text-sm font-semibold text-slate-700 mb-2">
              <User className="w-4 h-4 text-teal-600" /> Nama Lengkap Sesuai
              Ijazah
            </label>
            <input
              required
              ...
              onChange={(e) =>
                handleFieldChange("fullName", e.target.value.toUpperCase())
              }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input
              ...
              value={attendee.city || ""}
              onChange={(e) =>
                handleFieldChange("city", e.target.value.toUpperCase())
              }
              className="input-base"
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                handleFieldChange("province", e.target.value.toUpperCase())
              }
              className="input-base"
        ...
        <div className="border-t border-slate-100 pt-6 mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="flex items-center gap-2 text-sm font-semibold text-slate-700 mb-2">
              <BookOpen className="w-4 h-4 text-teal-600" /> Asal Instansi
              (Madrasah/Sekolah)
            </label>
            <input
              required
              ...
              onChange={(e) =>
                handleFieldChange("schoolName", e.target.value.toUpperCase())
              }
              className="input-base"
          ...
          <div>
            ...
            >
              ...
              <option value="Pendidikan Agama Islam (Dinas)">
                Pendidikan Agama Islam (Dinas)
              </option>
          ...
          <div className="flex flex-col md:flex-row gap-6 items-start md:items-center">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-slate-500 mb-4 font-medium tracking-wide">
                Latar Biru/Merah. Kemeja/Jas rapi. File asli akan otomatis
                dikompres ke ukuran optimal (
                <span className="text-teal-600 font-bold">&lt; 100KB</span>).
              </p>
