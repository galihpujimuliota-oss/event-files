        ...
        try {
          ...
          if (session?.user) {
            ...
            if (settings?.isRegistrationOpen === false) {
              ...
              setAuthError('Registrasi telah ditutup.');
              return;
            }
        ...
        <div className="space-y-4 pt-2">
          {isRegistrationOpen === false ? (
            <div className="bg-slate-100 border border-slate-200 rounded-xl p-4 text-center">
              <p className="font-bold text-slate-700 mb-1">Registrasi Ditutup</p>
              <p className="text-xs text-slate-500">Mohon maaf, sesi pendaftaran telah berakhir atau ditutup oleh Admin.</p>
              <button onClick={() => navigate('/')} className="mt-4 text-teal-600 font-medium text-sm hover:underline">Kembali ke Beranda</button>
            </div>
          ) : (
            <button
