...
export default function AdminScanner() {
  ...
  const [activeTab, setActiveTab] = useState<
    "DASHBOARD" | "SCAN" | "DATA" | "UNREGISTERED" | "QR" | "INFO" | "SETTINGS"
  >("DASHBOARD");
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const [rekapTab, setRekapTab] = useState<
    "HOTEL" | "LEGALISIR" | "SASH" | "DOUBLE" | "ERROR"
  >("HOTEL");
  ...
  const downloadBase64File = (base64Data: string, filename: string) => {
    try {
      ...
      alert(
        "Gagal mengunduh bukti otomatis. Silakan klik kanan gambar lalu pilih Simpan Gambar.",
      );
    }
  };
  ...
  const generateDigitalReceipt = (
    ...
    try {
      ...
      ctx.fillText(
        "Dokumen ini valid sebagai bukti pembayaran yang terverifikasi aman.",
        300,
        138,
      );
      ...
      let currentY = 195;
      drawRow("NO TRANSAKSI", transactionId, currentY);
      currentY += 45;
      drawRow("NAMA PESERTA", attendee.fullName || "-", currentY);
      currentY += 45;
      drawRow("NPK / NIM", attendee.npk || "-", currentY);
      currentY += 45;
      drawRow("PROGRAM STUDI", attendee.studyField || "-", currentY);
      currentY += 45;
      drawRow("INSTITUSI / SEKOLAH", attendee.schoolName || "-", currentY);
      currentY += 45;
      drawRow("JENIS PEMBAYARAN", paymentTitle, currentY);
      currentY += 45;
      drawRow("SENDER ACCOUNT", accountName, currentY);
      currentY += 45;
      drawRow("REKENING PENGIRIM", bankAccount, currentY);
      currentY += 45;
      ...
      ctx.textAlign = "center";
      ctx.fillText(
        `*SYSTEM-DIGITAL-PROOF-AUTH-${(attendee.id || "Npk").slice(0, 8).toUpperCase()}*`,
        300,
        762,
      );
  ...
  const handleAdminUploadProof = async (
    fieldName:
      | "paymentHotelProofUrl"
      | "paymentLegalisirProofUrl"
      | "paymentSashProofUrl",
    file: File,
  ) => {
    ...
    try {
      ...
      const updated = await store.updateAttendeeAdmin(selectedAttendee.id, {
        [fieldName]: dataUrl,
      });
      ...
      if (updated) {
        ...
        setAttendeesList((prev) =>
          prev.map((a) => (a.id === selectedAttendee.id ? updated : a)),
        );
        alert(
          "Berhasil mengunggah dan menyimpan file bukti pembayaran yang baru!",
        );
      } else {
  ...
  const handleViewAndDownloadProof = async (
    id: string,
    fileType: "HOTEL" | "LEGALISIR" | "SASH" | "DOUBLE",
  ) => {
    setIsLoadingFile(true);
    try {
      ...
      if (activeType === "DOUBLE") {
        if (
          full.paymentLegalisirProofUrl &&
          full.paymentLegalisirProofUrl !== "yes"
        ) {
          activeType = "LEGALISIR";
        } else if (
          full.paymentHotelProofUrl &&
          full.paymentHotelProofUrl !== "yes"
        ) {
          activeType = "HOTEL";
        } else if (
          full.paymentSashProofUrl &&
          full.paymentSashProofUrl !== "yes"
        ) {
          activeType = "SASH";
      ...
      setLightboxImage({
        url: base64Url,
        title: `${fileTitle} - ${full.fullName} (${full.npk})`,
  ...
  const handleSaveEdit = async () => {
    ...
    if (editForm.npk) {
      ...
      if (!allowed) {
        alert(
          "Pemberitahuan: Nomor NPK/Siaga yang Anda ketikkan tidak ditemukan di dalam database file NPK/Siaga (2769 peserta)!",
        );
        return;
      }
  ...
  const handleVerifyPaymentModal = async () => {
    ...
    addLog(
      `Verifikasi berhasil melalui modal untuk: ${selectedAttendee.fullName} (${selectedAttendee.npk})`,
    );
    updateStats();
  };
  ...
  const handleSyncLocalData = async () => {
    ...
    if (result.success) {
      setSyncStatus({
        status: "SUCCESS",
        message: `Berhasil memindahkan ${result.count} data pendaftaran lokal ke Supabase Cloud!`,
      });
      updateStats();
    } else {
      setSyncStatus({
        status: "ERROR",
        message: result.message || "Gagal menyinkronkan data.",
      });
    }
  };
  ...
  const updateStats = async () => {
    ...
    try {
      ...
      const all = Object.values(allDict).filter(
        (a) => a.isRegistered && a.npk !== "APP_SETTINGS",
      ); // Pastikan merekap yang udah register final
      ...
      const luringCount = all.filter(
        (a) =>
          String(a.attendanceType || "")
            .trim()
            .toUpperCase() === "LURING",
      ).length;
      const daringCount = all.filter(
        (a) =>
          String(a.attendanceType || "")
            .trim()
            .toUpperCase() === "DARING",
  ...
  const handleVerification = async (id: string) => {
    ...
    if (verifiedAttendee) {
      ...
      addLog(
        `Verifikasi berhasil melalui scan barcode untuk: ${verifiedAttendee.fullName} (${verifiedAttendee.npk})`,
      );
      await updateStats();
    } else {
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute(
      "download",
      `Data_Peserta_Yudisium_${new Date().toISOString().slice(0, 10)}.csv`,
    );
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
      ...
      <header className="bg-teal-700 text-white p-4 shadow-md sticky top-0 z-10">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <div
                className={`w-2 h-2 rounded-full ${isRegistrationOpen ? "bg-teal-400" : "bg-rose-400 animate-pulse"}`}
              />
              <span className="hidden xs:inline">
                {isRegistrationOpen ? "Form Dibuka" : "Form Ditutup"}
              </span>
            </button>
            <button
              onClick={() => updateStats()}
            ...
            <button
              ...
              className="flex items-center gap-1 bg-teal-800 hover:bg-teal-900 px-3 py-1.5 rounded-lg text-sm font-semibold transition-colors"
            >
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto p-4 md:p-6 pb-20 space-y-6">
        ...
        <div className="relative">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              {activeTab === "SCAN" && (
                <>
                  <ScanFace className="w-5 h-5 text-teal-600" /> SCANNER
                </>
              )}
              {activeTab === "DATA" && (
                <>
                  <Table2 className="w-5 h-5 text-teal-600" /> DATA PENDAFTARAN
                </>
              )}
              {activeTab === "UNREGISTERED" && (
                <>
                  <UserX className="w-5 h-5 text-amber-600" /> BELUM REGISTRASI
                </>
              )}
              {activeTab === "QR" && (
                <>
                  <QrCode className="w-5 h-5 text-teal-600" /> CETAK QR CODES
                </>
              )}
              {activeTab === "INFO" && (
                <>
                  <BellRing className="w-5 h-5 text-teal-600" /> BROADCAST INFO
                </>
              )}
              {activeTab === "SETTINGS" && (
                <>
                  <Settings className="w-5 h-5 text-teal-600" /> SETTINGS
                </>
              )}
            </div>
            <ChevronDown
              className={`w-5 h-5 text-slate-400 transition-transform ${isMenuOpen ? "rotate-180" : ""}`}
            />
          </button>
            ...
            {isMenuOpen && (
              ...
              >
                <button
                  ...
                  className={`p-4 text-left font-bold flex items-center gap-3 transition-colors ${activeTab === "DASHBOARD" ? "bg-teal-50 text-teal-700 border-l-4 border-teal-600" : "text-slate-600 hover:bg-slate-50"}`}
                >
                ...
                <button
                  ...
                  className={`p-4 text-left font-bold flex items-center gap-3 transition-colors ${activeTab === "SCAN" ? "bg-teal-50 text-teal-700 border-l-4 border-teal-600" : "text-slate-600 hover:bg-slate-50"}`}
                >
                ...
                <button
                  ...
                  className={`p-4 text-left font-bold flex items-center gap-3 transition-colors ${activeTab === "DATA" ? "bg-teal-50 text-teal-700 border-l-4 border-teal-600" : "text-slate-600 hover:bg-slate-50"}`}
                >
                ...
                <button
                  ...
                  className={`p-4 text-left font-bold flex items-center gap-3 transition-colors ${activeTab === "UNREGISTERED" ? "bg-amber-50 text-amber-700 border-l-4 border-amber-600" : "text-slate-600 hover:bg-slate-50"}`}
                >
                ...
                <button
                  ...
                  className={`p-4 text-left font-bold flex items-center gap-3 transition-colors ${activeTab === "QR" ? "bg-teal-50 text-teal-700 border-l-4 border-teal-600" : "text-slate-600 hover:bg-slate-50"}`}
                >
                ...
                <button
                  ...
                  className={`p-4 text-left font-bold flex items-center gap-3 transition-colors ${activeTab === "INFO" ? "bg-teal-50 text-teal-700 border-l-4 border-teal-600" : "text-slate-600 hover:bg-slate-50"}`}
                >
                ...
                <button
                  ...
                  className={`p-4 text-left font-bold flex items-center gap-3 transition-colors ${activeTab === "SETTINGS" ? "bg-teal-50 text-teal-700 border-l-4 border-teal-600" : "text-slate-600 hover:bg-slate-50"}`}
                >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <motion.div
            ...
            className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm text-center relative"
          >
            ...
            {stats.error > 0 && (
              <p
                className="text-[10px] text-rose-500 font-bold mt-1"
                title="Terdapat peserta yang data Kehadirannya tidak valid/kosong"
              >
                ⚠️ {stats.error} Error/Kosong
              </p>
            )}
          </motion.div>
          <motion.div
            ...
            className="bg-indigo-50 border border-indigo-100 rounded-xl p-4 shadow-sm text-center"
          >
          ...
          <motion.div
            ...
            className="bg-teal-50 border border-teal-100 rounded-xl p-4 shadow-sm text-center"
          >
          ...
          <motion.div
            ...
            className="bg-emerald-50 border border-emerald-100 rounded-xl p-4 shadow-sm text-center"
          >
            <p className="text-emerald-600 text-xs font-bold mb-1">
              TERVERIFIKASI
            </p>
        ...
        <AnimatePresence mode="wait">
            ...
            (() => {
              ...
              attendeesList.forEach((a) => {
                ...
                if (a.attendanceType === "LURING") {
                  luringProvinceCounts[prov] =
                    (luringProvinceCounts[prov] || 0) + 1;
              ...
              const verifiedCount = attendeesList.filter(
                (a) => a.status === "VERIFIED",
              ).length;

              // 4. Hotel proof vs Legalisir proof
              const hotelProofCount = attendeesList.filter(
                (a) => !!a.paymentHotelProofUrl,
              ).length;
              const legalisirProofCount = attendeesList.filter(
                (a) => !!a.paymentLegalisirProofUrl,
              ).length;
              ...
              const isSignificant = (s: string) =>
                s.length > 3 &&
                !["none", "null", "tidak ada", "-", "0", "na", "n/a"].includes(
                  s,
                );
              ...
              attendeesList.forEach((a) => {
                const npkRaw = norm(a.npk);
                if (isSignificant(npkRaw)) {
                  if (!npkMap.has(npkRaw)) npkMap.set(npkRaw, []);
                  npkMap.get(npkRaw)!.push(a);
                }
                const emailRaw = norm(a.email);
                if (isSignificant(emailRaw)) {
                  if (!emailMap.has(emailRaw)) emailMap.set(emailRaw, []);
                  emailMap.get(emailRaw)!.push(a);
                }
                const phoneRaw = norm(a.phoneWA);
                if (isSignificant(phoneRaw)) {
                  if (!phoneMap.has(phoneRaw)) phoneMap.set(phoneRaw, []);
                  phoneMap.get(phoneRaw)!.push(a);
                }
                const nameRaw = norm(a.fullName);
                if (isSignificant(nameRaw)) {
                  if (!nameMap.has(nameRaw)) nameMap.set(nameRaw, []);
                  nameMap.get(nameRaw)!.push(a);
                }
              });

              const duplicateInfo: Record<
                string,
                { reasons: string[]; isDuplicate: boolean }
              > = {};
              ...
              attendeesList.forEach((a) => {
                ...
                const npkRaw = norm(a.npk);
                if (isSignificant(npkRaw) && npkMap.get(npkRaw)!.length > 1)
                  npkMap.get(npkRaw)!.forEach((x) => potentialDups.add(x));
                const emailRaw = norm(a.email);
                if (
                  isSignificant(emailRaw) &&
                  emailMap.get(emailRaw)!.length > 1
                )
                  emailMap.get(emailRaw)!.forEach((x) => potentialDups.add(x));
                const phoneRaw = norm(a.phoneWA);
                if (
                  isSignificant(phoneRaw) &&
                  phoneMap.get(phoneRaw)!.length > 1
                )
                  phoneMap.get(phoneRaw)!.forEach((x) => potentialDups.add(x));
                const nameRaw = norm(a.fullName);
                if (isSignificant(nameRaw) && nameMap.get(nameRaw)!.length > 1)
                  nameMap.get(nameRaw)!.forEach((x) => potentialDups.add(x));
                ...
                for (const b of potentialDups) {
                  ...
                  if (
                    isSignificant(norm(a.fullName)) &&
                    norm(a.fullName) === norm(b.fullName)
                  )
                    reasons.push("Nama");
                  if (isSignificant(norm(a.npk)) && norm(a.npk) === norm(b.npk))
                    reasons.push("NPK");
                  if (
                    isSignificant(norm(a.email)) &&
                    norm(a.email) === norm(b.email)
                  )
                    reasons.push("Email");
                  if (
                    isSignificant(norm(a.phoneWA)) &&
                    norm(a.phoneWA) === norm(b.phoneWA)
                  )
                    reasons.push("No WA");
                  if (
                    isSignificant(norm(a.schoolName)) &&
                    norm(a.schoolName) === norm(b.schoolName)
                  )
                    reasons.push("Sekolah");
                  if (
                    isSignificant(norm(a.province)) &&
                    norm(a.province) === norm(b.province)
                  )
                    reasons.push("Provinsi");
                  if (
                    isSignificant(norm(a.city)) &&
                    norm(a.city) === norm(b.city)
                  )
                    reasons.push("Kota/Kab");
              ...
              const listHotel = attendeesList.filter(
                (a) =>
                  a.attendanceType === "LURING" ||
                  String(a.attendanceType).toLowerCase() === "luring",
              );
              const listLegalisir = attendeesList;
              const listSelempang = attendeesList.filter(
                (a) => a.wantsSash === true,
              );
              const listDouble = attendeesList.filter(
                (a) => duplicateInfo[a.id]?.isDuplicate,
              );
              const listError = attendeesList.filter((a) => {
                const t = String(a.attendanceType || "")
                  .trim()
                  .toUpperCase();
              ...
              const currentRekapPage = Math.min(
                rekapPage,
                totalRekapPages || 1,
              );
              ...
              const handleExportCategoryCSV = (
                category: "HOTEL" | "LEGALISIR" | "SASH" | "DOUBLE",
              ) => {
                let targetList = [];
                let catName = "";
                let headers = [
                ...
                const csvRows = [
                  ...
                  ...targetList.map((att, idx) => {
                    ...
                    const row = [String(idx + 1), att.fullName, att.npk];

                    row.push(att.phoneWA, bank, acctName, acctNum, proofUrl);
                ...
                link.setAttribute(
                  "download",
                  `Rekap_${catName}_${new Date().toISOString().slice(0, 10)}.csv`,
                );
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
              ...
              const copyAllWAOfActiveTab = () => {
                ...
                alert(
                  `Berhasil menyalin ${targetList.length} nomor WhatsApp ke clipboard!`,
                );
              };
              ...
              return (
                ...
                >
                  ...
                  <div className="bg-gradient-to-r from-teal-700 to-indigo-800 text-white rounded-2xl p-6 shadow-md flex flex-col md:flex-row items-center justify-between gap-6">
                    <div>
                      <h3 className="text-xl font-bold mb-1 flex items-center gap-2">
                        ...
                        Ringkasan Analitik Registrasi Yudisium
                      </h3>
                      <p className="text-xs text-teal-100/90 max-w-2xl leading-relaxed">
                        Visualisasi data pendaftaran, sebaran wilayah, program
                        studi peserta, status verifikasi administrasi finansial,
                        serta pemantauan kapasitas server secara real-time.
                      </p>
                    </div>
                    <div className="shrink-0 flex gap-2.5">
                      <button
                        onClick={handleExportCSV}
                        className="flex items-center gap-2 bg-white hover:bg-slate-100 text-teal-900 px-5 py-2.5 rounded-xl font-bold transition-all shadow-sm active:scale-95 text-xs"
                      >
                        <Download className="w-4 h-4 text-teal-700" /> Ekspor
                        CSV Penuh ({stats.total})
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4">
                      <h4 className="font-bold text-slate-800 text-sm flex items-center gap-2 pb-1 border-b border-slate-100">
                        <Building className="w-4 h-4 text-indigo-600" /> Top 5
                        Sebaran Asal Provinsi
                      </h4>
                      <div className="space-y-3.5">
                        {sortedProvinces.length === 0 ? (
                          <p className="text-xs text-slate-400 italic text-center py-6">
                            Belum ada data geografi wilayah.
                          </p>
                        ) : (
                          sortedProvinces.map(([prov, count], index) => {
                            const maxVal = Math.max(
                              ...Object.values(provinceCounts),
                              1,
                            );
                            const percentage =
                              Math.round((count / stats.total) * 100) || 0;
                    ...
                    <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-5">
                      <h4 className="font-bold text-slate-800 text-sm flex items-center gap-2 pb-1 border-b border-slate-100">
                        <Users className="w-4 h-4 text-amber-600" /> Rasio
                        Metode Kehadiran & Status
                      </h4>
                        ...
                        <div className="bg-slate-50 border border-slate-100 p-3.5 rounded-xl text-center space-y-2">
                          ...
                          <div className="w-full bg-slate-200 h-1.5 rounded-full overflow-hidden flex">
                            <div
                              ...
                              style={{
                                width: `${stats.total > 0 ? (stats.luring / stats.total) * 100 : 50}%`,
                              }}
                            />
                            <div
                              className="bg-teal-500 h-full"
                              style={{
                                width: `${stats.total > 0 ? (stats.daring / stats.total) * 100 : 50}%`,
                              }}
                            />
                          </div>
                        </div>
                        ...
                        <div className="bg-slate-50 border border-slate-100 p-3.5 rounded-xl text-center space-y-2">
                          ...
                          <div className="w-full bg-slate-200 h-1.5 rounded-full overflow-hidden flex">
                            <div
                              ...
                              style={{
                                width: `${stats.total > 0 ? (verifiedCount / stats.total) * 100 : 50}%`,
                              }}
                            />
                            <div
                              className="bg-amber-500 h-full"
                              style={{
                                width: `${stats.total > 0 ? (pendingCount / stats.total) * 100 : 50}%`,
                              }}
                            />
                          </div>
                        </div>
                      ...
                      <div className="bg-indigo-50/40 border border-indigo-100/60 rounded-xl p-3 text-xs flex justify-between items-center">
                        <span className="font-semibold text-slate-700">
                          Lampiran Administrasi Diunggah:
                        </span>
                    ...
                    <div className="md:col-span-2 bg-gradient-to-br from-indigo-50 to-white border border-indigo-100 rounded-2xl p-5 shadow-sm space-y-4">
                      <h4 className="font-bold text-indigo-900 text-sm flex items-center gap-2 pb-2 border-b border-indigo-100/50">
                        ...
                        Infografis Geografi Peserta LURING
                      </h4>
                      <p className="text-xs text-indigo-600/80 mb-4">
                        Sebaran spesifik domisili (Provinsi & Kabupaten/Kota)
                        bagi peserta yang memilih metode kehadiran tatap muka (
                        <strong className="font-bold">LURING</strong>).
                      </p>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="space-y-3.5">
                          <h5 className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-3">
                            Top Provinsi Asal Luring
                          </h5>
                          {sortedLuringProvinces.length === 0 ? (
                            <p className="text-xs text-slate-400 italic py-4">
                              Belum ada data pendaftar luring.
                            </p>
                          ) : (
                            sortedLuringProvinces.map(
                              ([prov, count], index) => {
                                const maxVal = Math.max(
                                  ...Object.values(luringProvinceCounts),
                                  1,
                                );
                                const percentage =
                                  Math.round(
                                    (count / (stats.luring || 1)) * 100,
                                  ) || 0;
                                const widthPercentage =
                                  Math.round((count / maxVal) * 100) || 0;
                                return (
                        ...
                        <div className="space-y-3.5">
                          ...
                          {sortedLuringCities.length === 0 ? (
                            ...
                            sortedLuringCities.map(([city, count], index) => {
                              const maxVal = Math.max(
                                ...Object.values(luringCityCounts),
                                1,
                              );
                              const percentage =
                                Math.round(
                  ...
                  <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-6">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-5">
                      <div>
                        <h3 className="font-extrabold text-slate-800 text-base flex items-center gap-2">
                          <Table2 className="w-5 h-5 text-teal-600" /> Rekap
                          Data Berdasarkan Kategori
                        </h3>
                        <p className="text-slate-500 text-xs mt-1">
                          Menu rekap instan peserta untuk Pembayaran Acara
                          Hotel, Pembayaran Legalisir, serta Pembayaran
                          Selempang.
                        </p>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        ...
                        >
                          <Mail className="w-4 h-4 text-slate-500" /> Salin
                          Semua No. WA ({getFilteredRekapList().length})
                        </button>
                        <button
                    ...
                    <div className="flex bg-slate-50 p-1.5 rounded-xl gap-1 border border-slate-100 max-w-fit flex-col sm:flex-row">
                      ...
                      >
                        🏨 PEMBAYARAN HOTEL
                        <span
                          className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${rekapTab === "HOTEL" ? "bg-indigo-50 text-indigo-700" : "bg-slate-200 text-slate-600"}`}
                        >
                          {listHotel.length}
                        </span>
                      </button>
                      ...
                      >
                        ...
                        <span
                          className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${rekapTab === "LEGALISIR" ? "bg-teal-50 text-teal-700" : "bg-slate-200 text-slate-600"}`}
                        >
                      ...
                      >
                        ...
                        <span
                          className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${rekapTab === "SASH" ? "bg-amber-50 text-amber-700" : "bg-slate-200 text-slate-600"}`}
                        >
                      ...
                      >
                        ⚠️ DATA ERROR
                        <span
                          className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${rekapTab === "ERROR" ? "bg-red-50 text-red-700" : "bg-slate-200 text-slate-600"}`}
                        >
                          {listError.length}
                        </span>
                      </button>
                      ...
                      <div className="relative w-full max-w-md border border-slate-200 rounded-xl overflow-hidden flex items-center bg-white transition-shadow focus-within:border-teal-500 focus-within:ring-1 focus-within:ring-teal-500 shadow-sm">
                        ...
                        {rekapSearch && (
                          <button
                            ...
                            className="absolute right-4 text-xs font-bold text-slate-405 hover:text-slate-600"
                          >
                            Batal
                          </button>
                        )}
                      ...
                      <button
                        onClick={() => {
                          ...
                          const rows = filteredRekap.map((att, i) => {
                            let detailsStr = "";
                            if (rekapTab === "HOTEL") {
                              detailsStr = att.paymentHotelAccountName
                                ? `${att.paymentHotelBank} an. ${att.paymentHotelAccountName} (${att.paymentHotelAccountNumber})`
                                : "";
                            } else if (rekapTab === "LEGALISIR") {
                              detailsStr = att.paymentLegalisirAccountName
                                ? `${att.paymentLegalisirBank} an. ${att.paymentLegalisirAccountName} (${att.paymentLegalisirAccountNumber})`
                                : "";
                            } else if (rekapTab === "SASH") {
                              detailsStr = att.paymentSashAccountName
                                ? `${att.paymentSashBank} an. ${att.paymentSashAccountName} (${att.paymentSashAccountNumber})`
                                : "";
                          ...
                          link.setAttribute(
                            "download",
                            `Rekap_Pembayaran_${rekapTab}_${new Date().toISOString().slice(0, 10)}.csv`,
                          );
                          document.body.appendChild(link);
                          link.click();
                          document.body.removeChild(link);
                      ...
                      <div className="max-h-[350px] overflow-y-auto">
                        <table className="w-full text-left text-xs border-collapse">
                          <thead className="bg-[#0f172a] text-white sticky top-0 z-10 shadow-sm">
                            <tr>
                              <th className="px-4 py-3 font-semibold text-[10px] tracking-wider uppercase text-slate-300 w-12 text-center">
                                No
                              </th>
                              <th className="px-4 py-3 font-semibold text-[10px] tracking-wider uppercase text-slate-300">
                                Nama Lengkap
                              </th>
                              <th className="px-4 py-3 font-semibold text-[10px] tracking-wider uppercase text-slate-300">
                                NPK / Akun Siaga
                              ...
                              <th className="px-4 py-3 font-semibold text-[10px] tracking-wider uppercase text-slate-300 text-center">
                                Bukti Pembayaran
                              </th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-slate-100 bg-white">
                            {paginatedRekap.map((att, index) => {
                              ...
                              if (rekapTab === "HOTEL") {
                                ...
                                proofUrl =
                                  att.paymentLegalisirProofUrl ||
                                  att.paymentHotelProofUrl ||
                              ...
                              const isDuplicate =
                                !!duplicateInfo[att.id]?.isDuplicate;

                              return (
                                <tr
                                  key={att.id}
                                  className={`hover:bg-slate-50/80 transition-colors ${isDuplicate ? "bg-rose-50/20" : ""}`}
                                >
                                  <td className="px-4 py-3 text-slate-400 font-mono text-center">
                                    {startRekapIndex + index + 1}
                                  </td>
                                  ...
                                  <td className="px-4 py-3 text-center">
                                    {proofUrl ? (
                                      ...
                                      <span className="text-rose-500 text-xs font-bold bg-rose-50 border border-rose-100 px-2.5 py-1 rounded inline-block">
                                        Belum Unggah
                                      </span>
                                    )}
                                  </td>
                            ...
                            {filteredRekap.length === 0 && (
                              <tr>
                                <td
                                  colSpan={5}
                                  className="px-5 py-12 text-center text-slate-400 font-medium italic"
                                >
                      ...
                      {totalRekapPages > 1 && (
                        <div className="flex items-center justify-between border-t border-slate-100 p-3 bg-slate-50/50 gap-4 flex-wrap">
                          <p className="text-[11px] text-slate-500 font-mono font-medium">
                            Menampilkan {startRekapIndex + 1} -{" "}
                            {Math.min(endRekapIndex, totalRekapItems)} dari{" "}
                            {totalRekapItems} data
                          </p>
                          <div className="flex items-center gap-1.5">
                            <button
                              ...
                              onClick={() =>
                                setRekapPage((prev) => Math.max(1, prev - 1))
                              }
                            ...
                            <div className="hidden xs:flex items-center gap-1">
                              {Array.from(
                                { length: totalRekapPages },
                                (_, i) => i + 1,
                              ).map((p) => {
                                if (
                                  totalRekapPages > 5 &&
                                  Math.abs(p - currentRekapPage) > 1 &&
                                  p !== 1 &&
                                  p !== totalRekapPages
                                ) {
                            ...
                            <button
                              ...
                              onClick={() =>
                                setRekapPage((prev) =>
                                  Math.min(totalRekapPages, prev + 1),
                                )
                              }
                              className="px-2.5 py-1.5 rounded-lg border border-slate-200 text-[11px] font-bold text-slate-600 bg-white hover:bg-slate-50 disabled:opacity-50 transition-colors"
                            >
          ...
          {activeTab === "SCAN" && (
            ...
            >
              <div className="space-y-6">
                <div className="bg-white p-4 border border-slate-200 rounded-xl shadow-sm relative">
                  <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2">
                    <ScanFace className="w-5 h-5 text-teal-600" /> Kamera
                    Scanner
                  </h3>
                  <div className="relative rounded-xl overflow-hidden aspect-square">
                    <div
                      id="qr-reader"
                      className="w-full h-full bg-slate-900 border-0 flex items-center justify-center relative z-0 [&>video]:object-cover [&>video]:w-full [&>video]:h-full"
                    ></div>

                    {/* Visual Status Indicator Overlay */}
                    <div
                      className={`absolute inset-0 z-10 pointer-events-none border-4 transition-all duration-500 ease-in-out ${
                ...
                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                  <form onSubmit={handleManualScan} className="flex gap-2">
                    ...
                    <button
                      type="submit"
                      disabled={!scanInput}
                      className="bg-teal-600 hover:bg-teal-700 disabled:bg-slate-300 text-white px-6 rounded-xl font-bold transition-all shadow-sm active:scale-95"
                    >
                      Verifikasi
                    </button>
                  </form>
              ...
              <div
                className={`relative border-2 ${scanResult.status === "IDLE" ? "border-dashed border-slate-300 bg-white" : scanResult.status === "SUCCESS" ? "border-teal-200 bg-teal-50/50" : "border-rose-200 bg-rose-50/50"} rounded-xl p-6 min-h-[300px] flex flex-col justify-center items-center text-center transition-colors shadow-sm`}
              >
                {scanResult.status === "IDLE" && (
                  <motion.div
                    initial={{ opacity: 0 }}
                ...
                {scanResult.status === "NOT_FOUND" && (
                  ...
                  >
                    ...
                    <p className="text-rose-600 text-sm mb-6">
                      QR Code tidak valid / peserta belum registrasi final.
                    </p>
                    <button
                ...
                {scanResult.status === "SUCCESS" && scanResult.attendee && (
                  <motion.div
                    ...
                    animate={{ scale: 1, opacity: 1 }}
                    className="w-full text-left bg-white p-6 rounded-2xl shadow-sm border border-teal-100"
                  >
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="grid grid-cols-2 gap-4 text-xs">
                        <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                          <p className="text-slate-400 font-mono tracking-widest uppercase mb-1">
                            Bidang Studi
                          </p>
                          <p className="font-semibold text-slate-800">
                            {scanResult.attendee.studyField}
                          </p>
                        </div>
                        <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                          <p className="text-slate-400 font-mono tracking-widest uppercase mb-1">
                          ...
                          <p className="font-semibold text-slate-800">
                            {scanResult.attendee.schoolName}
                          </p>
                        </div>
                        <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                          <p className="text-slate-400 font-mono tracking-widest uppercase mb-1">
                      ...
                      <div className="mt-6 flex justify-center">
                        <button
                          onClick={() => setScanResult({ status: "IDLE" })}
                          className="px-6 py-2.5 bg-slate-800 hover:bg-slate-900 text-white font-bold rounded-lg transition-colors text-sm shadow-md active:scale-95"
                        >
              ...
              <div className="md:col-span-2 bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
                <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2">
                  <BellRing className="w-5 h-5 text-teal-600" /> Log Aktivitas
                  Verifikasi
                </h3>
                <div className="space-y-3 max-h-[200px] overflow-y-auto pr-2">
                  {adminLogs.length === 0 ? (
                    ...
                    adminLogs.map((log) => (
                      <div
                        key={log.id}
                        className="flex justify-between items-start border-b border-slate-100 pb-2 last:border-0 last:pb-0"
                      >
                        <div>
          ...
          {activeTab === "DATA" &&
            (() => {
              ...
              const duplicateNpks = new Set(
                attendeesList
                  .map((a) => a.npk)
                  .filter((npk, index, arr) => arr.indexOf(npk) !== index),
              );
              ...
              return (
                <motion.div
                  ...
                  animate={{ opacity: 1, x: 0 }}
                  exit={{ opacity: 0 }}
                  className="bg-white border text-sm border-slate-200 rounded-xl overflow-hidden shadow-sm"
                >
                  <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex flex-col md:flex-row items-center justify-between gap-4">
                    ...
                    <button
                      onClick={handleExportCSV}
                      className="flex items-center gap-2 bg-slate-800 hover:bg-slate-900 text-white px-5 py-2.5 rounded-xl font-semibold transition-all shadow-sm active:scale-95 text-xs whitespace-nowrap"
                    >
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left">
                      <thead className="bg-[#0f172a] text-white">
                        <tr>
                          <th className="px-5 py-4 font-semibold text-xs tracking-wider uppercase text-slate-300">
                            Nama & NPK
                          </th>
                          <th className="px-5 py-4 font-semibold text-xs tracking-wider uppercase text-slate-300">
                            Email & WA
                          ...
                          <th className="px-5 py-4 font-semibold text-xs tracking-wider uppercase text-slate-300 text-right">
                            Aksi
                          </th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100">
                        {paginatedAttendees.map((att) => (
                          ...
                          >
                            <td className="px-5 py-4">
                              <div className="flex items-center gap-2">
                                ...
                                {duplicateNpks.has(att.npk) && (
                                  <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-rose-100 text-rose-700 tracking-wider">
                                    GANDA
                                  </span>
                                )}
                              </div>
                            ...
                            <td className="px-5 py-4">
                              <span
                                className={`inline-block px-2.5 py-1 rounded-md text-[10px] font-bold tracking-widest uppercase ${att.attendanceType === "LURING" ? "bg-indigo-50 border border-indigo-100 text-indigo-700" : "bg-teal-50 border border-teal-100 text-teal-700"}`}
                              >
                                {att.attendanceType}
                              </span>
                            </td>
                            <td className="px-5 py-4">
                              <span
                                className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[10px] font-bold tracking-widest uppercase ${att.status === "VERIFIED" ? "bg-emerald-50 border border-emerald-100 text-emerald-700" : "bg-amber-50 border border-amber-200 text-amber-700"}`}
                              >
                                {att.status === "VERIFIED" ? (
                            ...
                            <td className="px-5 py-4 text-right">
                              <button
                                onClick={() => handleOpenAttendeeDetail(att)}
                              ...
                              <button
                                ...
                                className="text-rose-500 hover:text-rose-700 hover:bg-rose-50 p-2 rounded-lg transition-colors ml-1"
                              >
                        ...
                        {filteredAttendees.length === 0 && (
                          <tr>
                            <td
                              colSpan={5}
                              className="px-5 py-12 text-center text-slate-400 font-medium tracking-wide"
                            >
                  ...
                  {totalDataPages > 1 && (
                    <div className="flex items-center justify-between border-t border-slate-100 p-4 bg-slate-50/50 gap-4 flex-wrap">
                      <p className="text-xs text-slate-505 font-medium font-mono">
                        Menampilkan {startDataIndex + 1} -{" "}
                        {Math.min(endDataIndex, totalDataItems)} dari{" "}
                        {totalDataItems} data
                      </p>
                      <div className="flex items-center gap-1.5">
                        <button
                          ...
                          onClick={() =>
                            setDataPage((prev) => Math.max(1, prev - 1))
                          }
                        ...
                        <div className="hidden xs:flex items-center gap-1">
                          {Array.from(
                            { length: totalDataPages },
                            (_, i) => i + 1,
                          ).map((p) => {
                            if (
                              totalDataPages > 5 &&
                              Math.abs(p - currentDataPage) > 1 &&
                              p !== 1 &&
                              p !== totalDataPages
                            ) {
                        ...
                        <button
                          ...
                          onClick={() =>
                            setDataPage((prev) =>
                              Math.min(totalDataPages, prev + 1),
                            )
                          }
          ...
          {activeTab === "UNREGISTERED" &&
            (() => {
              ...
              if (rekapSearch) {
                ...
                unregisteredList = unregisteredList.filter(
                  (u) =>
              ...
              const totalUnregisteredPages = Math.ceil(
                totalUnregisteredItems / itemsPerPage,
              );
              const currentUnregisteredPage = Math.min(
                unregisteredPage,
                totalUnregisteredPages || 1,
              );
              const startUnregisteredIndex =
                (currentUnregisteredPage - 1) * itemsPerPage;
              const endUnregisteredIndex =
                startUnregisteredIndex + itemsPerPage;
              const paginatedUnregistered = unregisteredList.slice(
                startUnregisteredIndex,
                endUnregisteredIndex,
              );
              ...
              const exportToCsv = () => {
                ...
                link.setAttribute(
                  "download",
                  `Rekap_Belum_Registrasi_${new Date().toISOString().slice(0, 10)}.csv`,
                );
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
                  ...
                  <div className="p-4 sm:p-5 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-amber-50/50">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 font-medium mt-1">
                        Daftar peserta yang ada di file acuan namun belum
                        melakukan registrasi online ({unregisteredList.length}{" "}
                        orang).
                      </p>
                    </div>
                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full md:w-auto">
                      ...
                      <button
                        onClick={exportToCsv}
                        className="w-full sm:w-auto shrink-0 flex items-center justify-center gap-1.5 bg-amber-600 hover:bg-amber-700 text-white px-3 py-2 rounded-lg text-xs font-bold transition-colors"
                      >
                    ...
                    <div className="flex items-start gap-2.5">
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-3 pt-3 border-t border-teal-200/50">
                          ...
                          <div className="bg-white/70 p-2.5 rounded-lg border border-teal-200/40 shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
                            <span className="text-amber-700 block text-[10px] font-bold uppercase tracking-wider">
                      ...
                      <tbody className="divide-y divide-slate-100 font-medium bg-white">
                        {paginatedUnregistered.map((u, idx) => (
                          ...
                          >
                            ...
                            <td className="px-5 py-4 font-mono text-slate-600 font-bold text-center">
                              {u.npk}
                            </td>
                          </tr>
                        ))}
                          ...
                          <tr>
                            ...
                            >
                              {rekapSearch
                                ...
                                : "SEMUA PESERTA TELAH MELAKUKAN REGISTRASI."}
                            </td>
                          </tr>
                        )}
                      </tbody>
                  ...
                  {totalUnregisteredPages > 1 && (
                    <div className="flex items-center justify-between border-t border-slate-100 p-3 bg-slate-50/50 gap-4 flex-wrap">
                      <p className="text-[11px] text-slate-500 font-mono font-medium">
                        ...
                        {Math.min(endUnregisteredIndex, totalUnregisteredItems)}{" "}
                        dari {totalUnregisteredItems} data
                      </p>
                      <div className="flex items-center gap-1.5">
                        <button
                          type="button"
                          ...
                          onClick={() =>
                            setUnregisteredPage((prev) => Math.max(1, prev - 1))
                          }
                          className="px-2.5 py-1.5 rounded-lg border border-slate-200 text-[11px] font-bold text-slate-600 bg-white hover:bg-slate-50 disabled:opacity-50 transition-colors"
                        >
                          Sebelumnya
                        ...
                        <div className="hidden xs:flex items-center gap-1">
                          {Array.from(
                            { length: totalUnregisteredPages },
                            (_, i) => i + 1,
                          ).map((p) => {
                            if (
                              totalUnregisteredPages > 5 &&
                              Math.abs(p - currentUnregisteredPage) > 1 &&
                              p !== 1 &&
                              p !== totalUnregisteredPages
                            ) {
                        ...
                        <button
                          type="button"
                          disabled={
                            currentUnregisteredPage === totalUnregisteredPages
                          }
                          onClick={() =>
                            setUnregisteredPage((prev) =>
                              Math.min(totalUnregisteredPages, prev + 1),
                            )
                          }
                          className="px-2.5 py-1.5 rounded-lg border border-slate-200 text-[11px] font-bold text-slate-600 bg-white hover:bg-slate-50 disabled:opacity-50 transition-colors"
                        >
          ...
          {activeTab === "QR" &&
            (() => {
              const filteredQRList = attendeesList.filter(
                (att) =>
                  att.fullName
                    .toLowerCase()
                    .includes(searchQuery.toLowerCase()) ||
              ...
              const handleDownloadAll = () => {
                ...
                if (container) {
                  toPng(container, {
                    ...
                    .catch((err) =>
                      console.error("Failed to download QR batch", err),
                    );
                }
              };
              ...
              const handleDownloadSingle = (id: string, name: string) => {
                ...
                if (qrElement) {
                  toPng(qrElement, {
                    ...
                    .catch((err) =>
                      console.error("Failed to download individual QR", err),
                    );
                }
              };
              ...
              return (
                ...
                >
                  <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-4">
                    <div>
                      ...
                      <p className="text-slate-500 text-sm">
                        Unduh QR Code peserta untuk cetak ID Card (Name Tag).
                      </p>
                    ...
                    <div className="flex gap-2 w-full md:w-auto self-end">
                      <button
                        ...
                        className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-900 text-white px-5 py-2.5 rounded-xl font-semibold transition-all shadow-sm active:scale-95 text-xs select-none"
                      >
                        <Printer className="w-4 h-4" /> Print Halaman (PDF)
                      </button>
                      <button
                        onClick={handleDownloadAll}
                        disabled={paginatedQRList.length === 0}
                        className="disabled:opacity-50 flex-1 md:flex-none flex items-center justify-center gap-2 bg-teal-600 hover:bg-teal-700 text-white px-5 py-2.5 rounded-xl font-semibold transition-all shadow-sm active:scale-95 text-xs select-none"
                      >
                        <Download className="w-4 h-4" /> Download Hal.{" "}
                        {currentQRPage}
                      </button>
                    </div>
                  </div>
                  ...
                  {filteredQRList.length === 0 ? (
                    ...
                    <>
                      <div
                        id="qr-batch-container"
                        className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 bg-slate-50/50 rounded-xl border border-slate-100 p-4"
                      >
                        {paginatedQRList.map((att) => (
                          <div
                            key={att.id}
                            id={`qr-card-${att.id}`}
                            className="bg-white p-4 rounded-xl border border-slate-200 flex flex-col items-center justify-center text-center shadow-sm relative group overflow-hidden"
                          >
                            <div className="mb-3 p-1">
                              <QRCodeSVG value={att.id} size={110} level="H" />
                            </div>
                            <p
                              className="font-bold text-xs text-slate-800 line-clamp-1 w-full uppercase"
                              title={att.fullName}
                            >
                              {att.fullName}
                            </p>
                            <p className="text-[10px] text-slate-400 font-mono tracking-widest mt-0.5 mb-2">
                              {att.npk}
                            </p>
                            <span
                              className={`inline-block w-full py-1 rounded-[4px] text-[9px] font-bold tracking-widest uppercase ${att.attendanceType === "LURING" ? "bg-indigo-50 border border-indigo-100 text-indigo-700" : "bg-teal-50 border border-teal-100 text-teal-700"}`}
                            >
                              {att.attendanceType}
                            </span>
                            ...
                            <div className="absolute inset-0 bg-white/80 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                              <button
                                onClick={() =>
                                  handleDownloadSingle(att.id, att.fullName)
                                }
                      ...
                      {totalQRPages > 1 && (
                        <div className="flex items-center justify-between border-t border-slate-100 mt-6 pt-4 gap-4 flex-wrap">
                          <p className="text-[11px] text-slate-500 font-mono font-medium">
                            Menampilkan {startQRIndex + 1} -{" "}
                            {Math.min(endQRIndex, totalQRItems)} dari{" "}
                            {totalQRItems} QR Code
                          </p>
                          <div className="flex items-center gap-1.5">
                            <button
                              type="button"
                              ...
                              onClick={() =>
                                setQrPage((prev) => Math.max(1, prev - 1))
                              }
                              className="px-2.5 py-1.5 rounded-lg border border-slate-200 text-[11px] font-bold text-slate-600 bg-white hover:bg-slate-50 disabled:opacity-50 transition-colors"
                            >
                              Sebelumnya
          ...
          {activeTab === "INFO" && (
            <motion.div
              ...
              animate={{ opacity: 1, x: 0 }}
              exit={{ opacity: 0 }}
              className="bg-white border border-slate-200 rounded-xl p-6 max-w-2xl shadow-sm mx-auto mt-8"
            >
              <div className="flex gap-4 items-center mb-6">
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-sm leading-relaxed">
                    Kirim instruksi acara, link streaming, atau pengingat jadwal
                    langsung ke email seluruh peserta.
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-6">
                <div className="bg-slate-50/50 p-5 rounded-xl border border-slate-100 space-y-4">
                  ...
                  <p className="text-sm font-semibold text-teal-700 bg-teal-50 px-3 py-1.5 rounded-lg inline-block border border-teal-100">
                    {stats.total} Peserta (Terdaftar Final)
                  </p>

                  <div className="border-t border-slate-100 pt-3">
                    <p className="text-xs font-bold text-slate-500 tracking-wider uppercase mb-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <button
                        ...
                        onClick={() => {
                          setBroadcastSubject(
                            "Link Streaming: Yudisium & Pengukuhan Guru Profesional PPG Daljab FITK UIN Malang",
                          );
                          setBroadcastBody(
                      ...
                      <button
                        ...
                        onClick={() => {
                          setBroadcastSubject(
                            "Instruksi & Tata Tertib Acara Yudisium PPG FITK UIN Malang",
                          );
                          setBroadcastBody(
                  ...
                  <div className="pt-2">
                    <label className="block text-xs font-bold text-slate-500 tracking-wide uppercase mb-1.5">
                      Subjek Email (Dapat Diedit)
                    </label>
                    <input
                      type="text"
                ...
                <button
                  ...
                  className="w-full bg-teal-600 text-white py-3.5 rounded-xl font-bold hover:bg-teal-700 transition-all shadow-[0_8px_20px_-4px_rgba(13,148,136,0.3)] hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-2"
                >
          ...
          {activeTab === "SETTINGS" && (
            ...
            >
              <div className="flex gap-4 items-center mb-6 border-b border-slate-100 pb-6">
                ...
                <div>
                  <h3 className="font-bold text-slate-800 text-lg tracking-tight">
                    Status Cloud Database
                  </h3>
                  <p className="text-slate-500 text-sm leading-relaxed">
                    Pengaturan koneksi Database (Supabase) untuk sinkronisasi
                    data.
                  </p>
                </div>
              </div>
                ...
                {isSupabaseConnected && (
                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-5 mt-4">
                    ...
                    <p className="text-xs text-slate-600 mb-4 leading-relaxed">
                      Jika Anda sebelumnya sudah memiliki data pendaftaran (dari
                      ujicoba luring/daring) yang tersimpan secara lokal di
                      browser ini, dan sekarang setelah mereset tabel di
                      Supabase data tersebut menjadi 0, silakan klik tombol di
                      bawah ini untuk{" "}
                      <strong>mengunggah dan menyinkronkan ulang</strong> semua
                      data tersebut ke database Supabase Cloud Anda secara
                      instan:
                    </p>
                ...
                {isSupabaseConnected && stats.total === 0 && (
                  <div className="bg-amber-50 rounded-xl p-4 border border-amber-200 mt-4 text-sm text-amber-800">
                    <span className="font-bold block mb-1">
                      ⚠️ Mengapa Data 0 Padahal Terhubung?
                    </span>
                    <p className="mb-2">
                      <strong>
                        Peringatan Supabase (RLS Disabled / Gagal Simpan):
                      </strong>{" "}
                      Jika Anda masih gagal menyimpan data pendaftaran atau ada
                      error RLS, metode terbaik adalah{" "}
                      <strong>mereset ulang tabelnya</strong>.
                    </p>
                    <p>
                      <strong>Solusi (Hard Reset):</strong> Buka{" "}
                      <strong>SQL Editor</strong> di Supabase Anda, copy dan
                      jalankan kode di bawah ini.{" "}
                      <em>
                        (Catatan: Pastikan Anda belum memiliki data penting,
                        karena ini akan membuat ulang tabel menjadi bersih dari
                        awal)
                      </em>
                    </p>
                ...
                {!isSupabaseConnected && (
                  <div className="bg-blue-50 border border-blue-100 rounded-xl p-5 text-blue-900 mt-6 shadow-sm">
                    <h4 className="font-bold mb-2 flex items-center gap-2">
                      Membutuhkan Koneksi Supabase
                    </h4>
                    <p className="text-sm opacity-90 mb-4">
                      Saat ini aplikasi menggunakan <strong>Local DB</strong>.
                      Jika perangkat direfresh atau diakses oleh pengguna lain,
                      datanya akan terpisah. Untuk menyimpan dan menyinkronkan
                      data secara realtime (CLOUD):
                    </p>
                    <ol className="list-decimal list-inside space-y-1 text-sm font-medium">
                      <li>
                        Buat akun dan project di <strong>Supabase</strong>
                      </li>
                      <li>
                        Buka menu{" "}
                        <strong>
                          AI Studio Sidebar &gt; Settings &gt; Variables
                          (Secrets)
                        </strong>
                      </li>
                      <li>
                        Tambahkan{" "}
                        <code className="bg-white px-2 py-0.5 rounded text-blue-700 mx-1">
                          VITE_SUPABASE_URL
                        </code>
                      </li>
                      <li>
                      ...
                      <li>
                        Tunggu beberapa saat, lalu Refresh halaman web ini.
                      </li>
                    </ol>
                  </div>
                )}
          ...
          {selectedAttendee && (
            <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex justify-center items-center p-4 md:p-6 overflow-y-auto">
              <motion.div
                ...
                className="bg-white rounded-2xl w-full max-w-4xl shadow-2xl overflow-hidden flex flex-col my-auto max-h-[90vh]"
              >
                ...
                <div className="p-5 overflow-y-auto bg-slate-50/30 flex-1">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="space-y-3 text-sm">
                          <div className="flex flex-col">
                            ...
                            {isEditMode ? (
                              <input
                                type="text"
                                className="input-base p-2 border border-slate-300 rounded mt-1 text-sm bg-white"
                          ...
                          <div className="flex flex-col">
                            ...
                            {isEditMode ? (
                              <input
                                ...
                                value={editForm.npk || ""}
                                onChange={(e) =>
                                  handleEditChange("npk", e.target.value)
                                }
                              />
                            ) : (
                              <span className="font-mono text-slate-700">
                                {selectedAttendee.npk}
                              </span>
                            )}
                          </div>
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <div className="flex flex-col">
                              ...
                              {isEditMode ? (
                                ...
                                <span
                                  className={`inline-block px-2 text-xs font-bold leading-6 rounded uppercase mt-0.5 w-fit ${selectedAttendee.attendanceType === "LURING" ? "bg-indigo-100 text-indigo-700" : "bg-teal-100 text-teal-700"}`}
                                >
                                  {selectedAttendee.attendanceType}
                                </span>
                              )}
                            </div>
                            <div className="flex flex-col">
                              ...
                              <span
                                className={`inline-block px-2 text-xs font-bold leading-6 rounded uppercase mt-0.5 w-fit ${selectedAttendee.status === "VERIFIED" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}
                              >
                          ...
                          <div className="flex flex-col">
                            ...
                            {isEditMode ? (
                              <input
                                ...
                                onChange={(e) =>
                                  handleEditChange("studyField", e.target.value)
                                }
                              />
                            ) : (
                              <span className="text-slate-700">
                                {selectedAttendee.studyField}
                              </span>
                            )}
                          </div>
                            ...
                            <div className="flex flex-col">
                              ...
                              {isEditMode ? (
                                <input
                                  ...
                                  onChange={(e) =>
                                    handleEditChange("phoneWA", e.target.value)
                                  }
                                />
                              ) : (
                                <span className="font-mono text-slate-700">
                                  {selectedAttendee.phoneWA}
                                </span>
                              )}
                            </div>
                          </div>
                    ...
                    <div className="space-y-6">
                      <div>
                        <h4 className="font-bold text-slate-700 text-sm border-b border-slate-200 pb-2 mb-4">
                          Informasi Pembayaran
                        </h4>
                        <div className="space-y-6">
                          {(selectedAttendee.attendanceType === "LURING" ||
                            selectedAttendee.paymentHotelProofUrl) && (
                            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                              <h5 className="font-bold text-slate-800 text-xs mb-3 uppercase tracking-wider">
                                Hotel (LURING)
                              </h5>
                              <div className="grid grid-cols-1 gap-2 text-sm text-slate-700 mb-4">
                                <p>
                                  ...
                                  {selectedAttendee.paymentHotelBank || "-"}
                                </p>
                                <p>
                                  <span className="text-slate-400 font-semibold block text-[10px] uppercase">
                                    No Rekening
                                  ...
                                  <span className="font-mono">
                                    {selectedAttendee.paymentHotelAccountNumber ||
                                      "-"}
                                  </span>
                                </p>
                                <p>
                                  ...
                                  {selectedAttendee.paymentHotelAccountName ||
                                    "-"}
                                </p>
                              </div>
                              ...
                              selectedAttendee.paymentHotelProofUrl !==
                                ...
                                <div className="border border-slate-200 rounded-lg overflow-hidden relative group">
                                  <img
                                    src={selectedAttendee.paymentHotelProofUrl}
                                    alt="Bukti Hotel"
                                  ...
                                  <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-all">
                                    <button
                                      onClick={() => {
                                        ...
                                        setLightboxImage({
                                          url: selectedAttendee.paymentHotelProofUrl as string,
                                          title: `Bukti Pembayaran Hotel - ${selectedAttendee.fullName} (${selectedAttendee.npk})`,
                                          filename,
                                        });
                                      }}
                                      className="bg-white hover:bg-slate-50 text-slate-800 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors"
                                    >
                                      Zoom/Buka Jelas
                                    </button>
                                  </div>
                                ...
                                <div className="border border-teal-200 rounded-lg overflow-hidden relative group shadow-sm">
                                  <div className="w-full h-24 bg-teal-50 flex flex-col items-center justify-center p-3 text-center transition-all group-hover:bg-teal-100">
                                    ...
                                    <span className="text-[10px] text-teal-600 mt-1 leading-snug">
                                      Terverifikasi otomatis di database server.
                                      Klik di bawah untuk mencetak kuitansi.
                                    </span>
                                  </div>
                                  <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-all">
                                    <button
                                      onClick={() => {
                                        ...
                                        setLightboxImage({
                                          url: generateDigitalReceipt(
                                            selectedAttendee,
                                            "HOTEL",
                                          ),
                                          title: `Bukti Hotel (Sistem) - ${selectedAttendee.fullName} (${selectedAttendee.npk})`,
                                          filename,
                                        });
                                      }}
                                      className="bg-teal-700 hover:bg-teal-800 text-white px-3 py-1.5 rounded-lg text-xs font-bold transition-colors shadow-md"
                                    >
                                      Lihat Kuitansi
                                    </button>
                                  </div>
                                ...
                                <div className="h-20 border border-dashed border-slate-300 rounded-lg flex items-center justify-center text-slate-400 text-xs font-semibold">
                          ...
                          <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                            ...
                            <div className="grid grid-cols-1 gap-2 text-sm text-slate-700 mb-4">
                              <p>
                                ...
                                {selectedAttendee.paymentLegalisirBank || "-"}
                              </p>
                              <p>
                                <span className="text-slate-400 font-semibold block text-[10px] uppercase">
                                  No Rekening
                                ...
                                <span className="font-mono">
                                  {selectedAttendee.paymentLegalisirAccountNumber ||
                                    "-"}
                                </span>
                              </p>
                              <p>
                                ...
                                {selectedAttendee.paymentLegalisirAccountName ||
                                  "-"}
                              </p>
                            </div>
                                ...
                                <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-all">
                                  <button
                                    onClick={() => {
                                      ...
                                      setLightboxImage({
                                        url: selectedAttendee.paymentLegalisirProofUrl as string,
                                        title: `Bukti Pembayaran Legalisir - ${selectedAttendee.fullName} (${selectedAttendee.npk})`,
                                        filename,
                                      });
                                    }}
                                    className="bg-white hover:bg-slate-50 text-slate-800 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors"
                                  ...
                                  <button
                                    onClick={() => {
                                      ...
                                      setLightboxImage({
                                        ...
                                        title: `Bukti Legalisir (Sistem) - ${selectedAttendee.fullName} (${selectedAttendee.npk})`,
                                        filename,
                                      });
                                    }}
                                    className="bg-teal-700 hover:bg-teal-800 text-white px-3 py-1.5 rounded-lg text-xs font-bold transition-colors shadow-md"
                            ...
                            {selectedAttendee.wantsSash ? (
                              <div className="space-y-4">
                                <div className="grid grid-cols-1 gap-2 text-xs text-slate-700">
                                  <p>
                                    <span className="text-slate-400 font-bold block text-[9px] uppercase tracking-wider mb-0.5">
                                      Bank Pengirim
                                    </span>
                                    {selectedAttendee.paymentSashBank || "-"}
                                  </p>
                                  <p>
                                    <span className="text-slate-400 font-bold block text-[9px] uppercase tracking-wider mb-0.5">
                                      No Rekening
                                    ...
                                    <span className="font-mono">
                                      {selectedAttendee.paymentSashAccountNumber ||
                                        "-"}
                                    </span>
                                  </p>
                                  <p>
                                    ...
                                    {selectedAttendee.paymentSashAccountName ||
                                      "-"}
                                  </p>
                                </div>
                                ...
                                selectedAttendee.paymentSashProofUrl !==
                                  ...
                                  <div className="border border-slate-200 rounded-lg overflow-hidden relative group">
                                    <img
                                      src={selectedAttendee.paymentSashProofUrl}
                                      alt="Bukti Selempang"
                                      className="w-full h-32 object-contain bg-slate-100"
                                    />
                                    <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-all">
                                      <button
                                        onClick={() => {
                                          ...
                                          setLightboxImage({
                                            url: selectedAttendee.paymentSashProofUrl as string,
                                            title: `Bukti Pembayaran Selempang - ${selectedAttendee.fullName} (${selectedAttendee.npk})`,
                                            filename,
                                          });
                                        }}
                                        className="bg-white hover:bg-slate-50 text-slate-800 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors"
                                  ...
                                  <div className="border border-teal-200 rounded-lg overflow-hidden relative group shadow-sm bg-white">
                                    <div className="w-full h-24 bg-teal-50 flex flex-col items-center justify-center p-3 text-center transition-all group-hover:bg-teal-100">
                                      ...
                                      <span className="text-[10px] text-teal-600 mt-1 leading-snug">
                                        Terverifikasi otomatis di database
                                        server. Klik di bawah untuk mencetak
                                        kuitansi.
                                      </span>
                                    </div>
                                    <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-all">
                                      <button
                                        onClick={() => {
                                          ...
                                          setLightboxImage({
                                            ...
                                            title: `Bukti Selempang (Sistem) - ${selectedAttendee.fullName} (${selectedAttendee.npk})`,
                                            filename,
                                          });
                                        }}
                                        className="bg-teal-700 hover:bg-teal-800 text-white px-3 py-1.5 rounded-lg text-xs font-bold transition-colors shadow-md"
                                  ...
                                  <div className="h-20 border border-dashed border-rose-300 rounded-lg flex items-center justify-center text-rose-500 text-xs font-semibold">
                                    Bukti Belum Diunggah
                                  </div>
                                )}
                              ...
                              <div className="py-4 text-center text-slate-400 text-xs font-medium italic border border-dashed border-slate-200 rounded-lg">
                                Peserta tidak memesan selempang
                              </div>
                            )}
                          </div>
                          ...
                          {selectedAttendee.attendanceType === "DARING" && (
                            <div className="bg-indigo-50 border border-indigo-100 p-4 rounded-xl text-indigo-900 shadow-sm">
                              <h5 className="font-bold text-indigo-800 text-xs mb-2 uppercase tracking-wider">
                                Pengambilan Serdik
                              </h5>
                              <p className="text-sm font-semibold">
                                {getCertMethodText(
                                  selectedAttendee.certificateRetrievalMethod,
                                )}
                              </p>
                              {selectedAttendee.certificateRetrievalMethod ===
                                "MODEL_3" && (
                                <p className="text-xs text-indigo-600 mt-2 italic">
                                  * Harus mengisi Form IKA PPG
                                </p>
                              )}
                            </div>
                          )}
                ...
                <div className="p-4 md:p-5 border-t border-slate-200 bg-white flex flex-col sm:flex-row justify-end gap-3 shrink-0">
                  {isEditMode ? (
                    <button
                      ...
                      className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl text-sm transition-colors shadow-sm order-1 sm:order-2"
                    >
                      Simpan Update
                    </button>
                  ) : (
                    <>
                      <button
                        onClick={() => setSelectedAttendee(null)}
                        className="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl text-sm transition-colors order-2 sm:order-1"
                      >
                        Tutup
                      </button>
                      {selectedAttendee.status === "PENDING" && (
                        <button
                          onClick={handleVerifyPaymentModal}
                          className="px-5 py-2.5 bg-teal-600 hover:bg-teal-700 text-white font-bold rounded-xl text-sm flex items-center justify-center gap-2 transition-colors shadow-sm order-1 sm:order-2"
                        >
          ...
          {lightboxImage && (
            ...
            >
              ...
              <div className="w-full max-w-4xl flex items-center justify-between text-white mb-4 z-10 animate-in fade-in slide-in-from-top-4 duration-200">
                <div className="flex flex-col">
                  ...
                  <p className="text-slate-400 text-[10px] md:text-xs">
                    Lihat & simpan pas foto / bukti pembayaran dengan kualitas
                    asli
                  </p>
                </div>
                <div className="flex items-center gap-2">
              ...
              <div className="mt-4 text-slate-400 text-center text-[11px] font-medium max-w-md">
                Klik tombol{" "}
                <span className="text-teal-400 font-bold">Unduh Berkas</span>{" "}
                untuk menyimpan bukti secara langsung. Anda juga dapat klik
                kanan atau tekan lama gambar untuk menyimpan.
              </div>
            </motion.div>
          )}
        </AnimatePresence>
